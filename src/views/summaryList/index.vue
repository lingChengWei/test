  ...
  <div class="index">
    ...
    <div class="main" style="">
      ...
      >
        ...
        <el-table-column prop="id" width="100" label="宝贝id"></el-table-column>
        <el-table-column label="表格上传内容">
          <template slot-scope="scope">
            <div>
              <div>文本1：{{ scope.row.comment1 }}</div>
              <div>文本2：{{ scope.row.comment2 }}</div>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    methods: {
      ...
      async getCGoods(params) {
        ...
        const data = {
          ...
          // page: params.page,
          product_type: this.formData.cycle,
        };
        try {
          this.loading = true;
          const res = await getCGoods(data);
